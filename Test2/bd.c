...
void printf_train_info(Train train) {
...
void enter_train_info(Train *train) {
	char c[100], sym;
	printf("Номер поезда: ");
    ...
    printf("\n");
        ...
        printf("--------------------------------------------------------------\n");
				...
				if (count_find > 0) {
					...
					while (j < count_find) {
						fseek(fp, sizeof(Train) * train_for_delete[j], SEEK_SET);
            ...
            printf("Номер записи: %d\n", j+1);
					...
					tp = fopen("tmp", "wb");
					fseek(fp, sizeof(Train) * train_for_delete[delete_train - 1], SEEK_SET);
					tell = ftell(fp);
					fseek(fp, 0, SEEK_SET);
					while (fread(&tmp, sizeof(Train), 1, fp) != 0) {
						cur_tell = ftell(fp);
						if (cur_tell != tell + sizeof(Train))
							fwrite(&tmp, sizeof(Train), 1, tp);
					}
					fclose(tp);
					fclose(fp);
					remove(filename);
